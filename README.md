<!-- Animated Header Section -->
<div align="center">
  <img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/aqua.png" width="100%">
  
  <h1>Hi 👋, I'm Ritik Pandey</h1>
  
  <h3>🚀 Software Engineer | Java Backend Developer | Spring Boot | FastAPI | Cloud Enthusiast</h3>
  
  <p>
    <b>Building scalable backend systems, robust REST APIs, and production-ready distributed microservices.</b>
  </p>
  
  <!-- Typing Effect Banner -->
  <p>
    <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=24&pause=1000&color=00C2FF&center=true&vCenter=true&width=750&lines=Software+Engineer;Java+Backend+Developer;Spring+Boot+Developer;FastAPI+Developer;Cloud+Enthusiast;Always+Learning+New+Technologies"/>
  </p>

  <!-- Visitor Counter and Social Stats -->
  <p>
    <img src="https://komarev.com/ghpvc/?username=RitikPandey68&label=Profile%20Views&color=0e75b6&style=for-the-badge" alt="Profile Views"/>
    <img src="https://img.shields.io/github/followers/RitikPandey68?style=for-the-badge" alt="Followers"/>
    <img src="https://img.shields.io/github/stars/RitikPandey68?style=for-the-badge" alt="Stars"/>
  </p>
  
  <!-- Social Quick Links -->
  <p>
    <a href="https://linkedin.com/in/ritik-pandey-sde/"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"></a>
    <a href="mailto:ritikpandeycode@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"></a>
    <a href="https://leetcode.com/u/ritikpandeycode/"><img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black" alt="LeetCode"></a>
    <a href="https://www.hackerrank.com/profile/pandeyritikkuma1"><img src="https://img.shields.io/badge/HackerRank-2EC866?style=for-the-badge&logo=hackerrank&logoColor=white" alt="HackerRank"></a>
  </p>

  <img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/aqua.png" width="100%">
</div>

---

# 💫 About Me

* 💻 **Backend-focused Software Engineer** passionate about building robust, high-performance, and scalable software architectures.
* 🚀 **Experienced** in writing clean code with **Java**, **Spring Boot**, **Python**, **FastAPI**, **Node.js**, REST APIs, and database engines.
* 🧠 **Strong foundations** in Data Structures & Algorithms (solved **300+** problems across LeetCode & HackerRank).
* ☁️ **Cloud Practitioner** interested in Distributed Systems, Microservices orchestration, and automated CI/CD pipelines.
* 🏆 **Credentials**: AWS Certified Cloud Practitioner | Google Cloud Innovators Member.
* 🎯 **Goal**: Actively seeking Software Engineer / Backend Developer opportunities where I can solve complex engineering challenges.

---

# 🚀 Tech Stack

### 💻 Programming Languages
<p>
  <img src="https://skillicons.dev/icons?i=java,python,cpp,js,ts,c,html,css" alt="Languages"/>
</p>

### ⚙️ Backend Development
<p>
  <img src="https://skillicons.dev/icons?i=spring,fastapi,nodejs" alt="Backend"/>
</p>

### 🗄️ Databases & Caching
<p>
  <img src="https://skillicons.dev/icons?i=postgres,mysql,mongodb,redis" alt="Databases"/>
</p>

### ☁️ Cloud & DevOps
<p>
  <img src="https://skillicons.dev/icons?i=aws,docker,kubernetes,linux,git,github" alt="DevOps"/>
</p>

---

# ⭐ Featured Projects

*Below is a showcase of production-ready and enterprise-grade repositories. Click on each card to expand the full recruiter brief.*

<br>

<!-- Project 1 -->
<details>
  <summary><b>🛠️ Project 1: NovaITSM — Enterprise ITIL Service Desk & ServiceNow Sandbox</b></summary>
  <br>
  
  ### 📌 Overview
  * **Problem Statement**: Enterprise IT operations struggle with fragmented ticketing systems, delayed SLA resolutions, lack of real-time query visibility, and decoupled testing environments for ServiceNow developers.
  * **What it does**: A production-grade **ITIL v4 Service Desk Portal** and **ServiceNow simulation engine** incorporating incident/problem workflows, SLA monitoring counters, CMDB tracking, and Active Directory lockout overrides.
  * **Tech Stack**: `Python`, `FastAPI`, `PostgreSQL` (ITSM Tickets), `MySQL` (CMDB Assets), `Redis` (Lockout Security), `Chart.js`, `Vanilla CSS` (Glassmorphism), `Docker Compose`.
  * **Key Features**:
    * **Operations Dashboard**: Real-time SLA tracking alerts and automatic MTTR (Mean Time to Resolve) calculation statistics.
    * **ITIL Queues**: Dynamic incident submission pipelines, problem management logs, and standard CMDB catalogs.
    * **Active Directory Security**: Enforces brute-force lockout defenses in Redis after 3 consecutive failed logins with admin overrides.
    * **ServiceNow PDI Sandbox**: Interprets client scripts (`g_form`), UI policies, GlideSystem business rules, and OUTBOUND REST integrations dynamically.
  * **Architecture Highlights**:
    * Clean multi-database routing configuration utilizing PostgreSQL for write-heavy tickets and MySQL for static CMDB records.
    * High-efficiency Redis-backed lockout keys to minimize hit loads on relational databases.
  * **Why this project is valuable**: Shows full-stack mastery of complex enterprise workflows, multi-database transaction layouts, and scripting simulators.
  * **🔗 Repository**: [Nova-Itsm-Service-Desk](https://github.com/RitikPandey68/Nova-Itsm-Service-Desk)
</details>

<!-- Project 2 -->
<details>
  <summary><b>🤖 Project 2: ResumeAI Pro — AI ATS Resume Analyzer</b></summary>
  <br>
  
  ### 📌 Overview
  * **Problem Statement**: Job seekers struggle to bypass automated Applicant Tracking Systems (ATS) because they lack deep insights into keyword indexing gaps, parser errors, and overall selection metrics.
  * **What it does**: An AI-powered web platform parsing candidate resumes, matching them against job descriptions, calculating ATS scores, and suggesting skill enhancements.
  * **Tech Stack**: `Python`, `FastAPI`, `NLP` (Natural Language Processing), `Machine Learning`, `Vanilla HTML5/CSS3/JavaScript`.
  * **Key Features**:
    * **Multi-Dimensional ATS Engine**: Simulates enterprise parser rules (e.g. Taleo, Workday) to compute fit scores.
    * **Section Match Analysis**: Side-by-side display of resume parts versus target job criteria.
    * **Keyword Intelligence & Skill Gaps**: Highlights missing professional terms and auto-generates learning path lists.
    * **Grammar Quality Check**: NLP check of writing tone, clarity, and structural issues.
    * **Interactive Dashboard & PDF Export**: Responsive control center with downloadable analysis reports.
  * **Architecture Highlights**:
    * Asynchronous text extraction pipeline converting heavy binary PDF data into token patterns for vector analysis.
    * Decoupled backend architecture providing rapid NLP analysis responses via FastAPI routes.
  * **Why this project is valuable**: Solves resume formatting visibility, demonstrating deep integration of AI models and backend parsing APIs.
  * **🔗 Repository**: [Ai-Resume-Analyzer](https://github.com/RitikPandey68/Ai-Resume-Analyzer)
</details>

<!-- Project 3 -->
<details>
  <summary><b>💰 Project 3: Personal Finance Tracker — Secure Expense Management</b></summary>
  <br>
  
  ### 📌 Overview
  * **Problem Statement**: Users find it difficult to manage personal budgets, track asset growth, and calculate unified financial health ratings across different transaction categories.
  * **What it does**: A fully secured, production-ready expense tracker and portfolio dashboard supporting real-time alerts, budgeting rules, and financial visualization.
  * **Tech Stack**: `Java`, `Spring Boot`, `Spring Security` (JWT), `JPA/Hibernate`, `PostgreSQL`, `Chart.js`, `Vanilla CSS`.
  * **Key Features**:
    * **Transaction Ingestion**: Multi-category entry records with automatic classification systems.
    * **Limit Warning Engine**: Automated triggers warning users visually when categories approach budget thresholds.
    * **Portfolio Tracker**: Comprehensive ledger storing mutual funds, stocks, and fixed deposit assets.
    * **Health Scoring Algorithm**: Custom metric scoring calculating debt-to-income and savings performance ratios.
    * **EMI & Bill Reminders**: Automatic calendar ledger alerting users of approaching deadlines.
  * **Architecture Highlights**:
    * State-of-the-art token security using JWT authentication mappings.
    * High-performance relational database indexes mapping accounts to transactions for fast ledger compilation.
  * **Why this project is valuable**: Shows strong Java capabilities, secure API designs, and comprehensive backend validation.
  * **🔗 Repository**: [Personal-Finance-Tracker](https://github.com/RitikPandey68/Personal-Finance-Tracker)
</details>

<!-- Project 4 -->
<details>
  <summary><b>💬 Project 4: AI Customer Support Chatbot — Semantic RAG Pipeline</b></summary>
  <br>
  
  ### 📌 Overview
  * **Problem Statement**: Legacy support systems fail to respond instantly, and generic chatbots cannot access contextual knowledgebases or route frustrated users to human agents.
  * **What it does**: A Retrieval-Augmented Generation (RAG) Customer Support Chatbot matching inquiries using vector search, detecting sentiment, and rendering dashboard controls.
  * **Tech Stack**: `Python`, `FastAPI`, `SQLAlchemy`, `PostgreSQL` (pgvector), `Redis`, `Chart.js`, `Vanilla HTML/CSS/JS`.
  * **Key Features**:
    * **Simulated RAG Search**: Computes similarity scores between customer questions and vector-indexed knowledge blocks.
    * **NLP Sentiment Escalation**: Monitors customer frustration levels, auto-routing high-risk sessions to human support.
    * **Widget Customizer Panel**: Admins can customize bot names, greetings, theme hex-codes, and avatar images dynamically.
    * **Support Analytics**: Renders ticket volume trends and sentiment stats via interactive Chart.js widgets.
  * **Architecture Highlights**:
    * Enforces pgvector indexes (GiST) inside PostgreSQL to search content chunks instantly.
    * Message tables partitioned declaratively on a monthly range basis to scale with millions of records.
  * **Why this project is valuable**: Combines semantic embeddings with high-speed indexing and automated alert escalations.
  * **🔗 Repository**: [Ai-Customer-Support-Chatbot](https://github.com/RitikPandey68/Ai-Customer-Support-Chatbot)
</details>

<!-- Project 5 -->
<details>
  <summary><b>🛒 Project 5: E-Commerce Platform — Transactional Storefront</b></summary>
  <br>
  
  ### 📌 Overview
  * **Problem Statement**: Designing secure storefronts requires managing complex product filter combinations, persisting user shopping carts, and ensuring transaction consistency.
  * **What it does**: A fully responsive multi-category storefront displaying product catalogs, search filtering, session cart caching, and security setups.
  * **Tech Stack**: `Java`, `Spring Boot`, `JPA/Hibernate`, `Spring Security` (JWT), `PostgreSQL`, `Vanilla JS`.
  * **Key Features**:
    * **Dynamic Catalogs**: Dynamic grid indexing 24+ products across Fashion, Tech, and Shoes.
    * **Multi-Tier Filters**: Sidebars for real-time cost sliders, product review ratings, and brand checkers.
    * **Stateful Checkout Session**: Keeps user cart items secure across browser resets.
    * **Flash Sale Synchronizer**: Dynamic timing script managing campaign countdown events.
    * **Multi-Auth Gates**: Secure user creation forms alongside Mock OAuth interfaces.
  * **Architecture Highlights**:
    * Decoupled backend entity design separating products, cart items, orders, and users.
    * Secure database connection pools to manage database request load spikes gracefully.
  * **Why this project is valuable**: Serves as a prime example of transactional consistency, search filters, and session management.
  * **🔗 Repository**: [Ecommerce-Website](https://github.com/RitikPandey68/Ecommerce-Website)
</details>

<!-- Project 6 -->
<details>
  <summary><b>📸 Project 6: Face Recognition Attendance System — Dual Mode Biometrics</b></summary>
  <br>
  
  ### 📌 Overview
  * **Problem Statement**: Traditional attendance tracking via registers or scanners is slow, error-prone, and lacks visual reports or simple registration steps.
  * **What it does**: A face recognition check-in platform simulating camera feeds, identifying registered faces, logging timings, and plotting compliance.
  * **Tech Stack**: `Python`, `FastAPI`, `SQLAlchemy`, `OpenCV`, `DeepFace / face_recognition`, `SQLite/PostgreSQL`, `Chart.js`.
  * **Key Features**:
    * **Three-Tier Landing Paths**: Custom interfaces customized for corporate, college, or school setups.
    * **HUD Camera Simulator**: Interactive camera frames scanning facial targets in real time.
    * **Facial Registration Portal**: Easily register new profiles by upload or snap, tagging branch and semester fields.
    * **Low-Attendance Triggers**: Sends alerts automatically to members falling below compliance lines.
    * **Reporting Panel**: Interactive bar and line charts reflecting weekly attendance records.
  * **Architecture Highlights**:
    * Multi-threading handling to process frame feeds without locking FastAPI thread pools.
    * Isolated database schema representing students, check-in records, and system settings.
  * **Why this project is valuable**: Combines computer vision models with real-world enterprise scheduling and reporting databases.
  * **🔗 Repository**: [Attendance-Tracking-Systems](https://github.com/RitikPandey68/Attendance-Tracking-Systems)
</details>

<!-- Project 7 -->
<details>
  <summary><b>📈 Project 7: Stock Market Dashboard — WebSocket & Time-Series Ledger</b></summary>
  <br>
  
  ### 📌 Overview
  * **Problem Statement**: Storing and displaying high-frequency financial tickers causes write bottlenecks, massive storage inflation, and lagging dashboards on typical relational database setups.
  * **What it does**: A high-performance trading dashboard consuming real-time stock ticks, charting prices, executing orders, and compiling assets.
  * **Tech Stack**: `Java`, `Spring Boot`, `TimescaleDB` (PostgreSQL Time-Series), `Spring WebSockets`, `Redis Cache`, `Chart.js`.
  * **Key Features**:
    * **Live WebSocket Feed**: Streams active ticker cost adjustments directly to client displays every 4 seconds.
    * **Trading HUD**: Execute instant simulated Buy/Sell orders on tickers.
    * **Holdings Compiler**: Computes average entry pricing, current asset evaluations, and real-time gain margins.
    * **Interactive Candlesticks**: Displays historical price shifts using Chart.js configurations.
    * **Curated News Cards**: Feeds the terminal with matching market news widgets.
  * **Architecture Highlights**:
    * Employs TimescaleDB hypertable partitioning on transaction timestamps.
    * Standardizes compression policies to shrink records older than 7 days, reducing disk footprint by up to 90%.
  * **Why this project is valuable**: Exhibits expertise in high-frequency writes, data partitioning, and real-time push engines (WebSockets).
  * **🔗 Repository**: [stock-market-dashboard](https://github.com/RitikPandey68/stock-market-dashboard)
</details>

<!-- Project 8 -->
<details>
  <summary><b>💼 Project 8: Job Portal — LinkedIn-Style Platform</b></summary>
  <br>
  
  ### 📌 Overview
  * **Problem Statement**: Job listings are spread thin across dozens of platforms, forcing candidates to create many profiles and manage applications across inconsistent links.
  * **What it does**: A unified job listing portal scraping simulated listings from major sites, presenting details in-app, and allowing rapid applications.
  * **Tech Stack**: `Java`, `Spring Boot`, `JPA/Hibernate`, `Spring Security` (JWT), `PostgreSQL`, `Vanilla HTML/CSS/JS`.
  * **Key Features**:
    * **Simulated Aggregator**: Consolidates postings from LinkedIn, Indeed, and Naukri into one unified catalog.
    * **In-Portal Details Drawer**: Read full descriptions and requirements without page redirects.
    * **One-Click Apply Indicator**: Tracks user applications and bookmarks.
    * **Employer Posting Wizard**: Job posting portal for hiring admins.
    * **JWT Token Security**: Secured endpoints for candidate profiles.
  * **Architecture Highlights**:
    * Employs PostgreSQL partitioning on publication dates to optimize search times.
    * Composite indices on job category, location tags, and post dates to ensure fast listings queries.
  * **Why this project is valuable**: Demonstrates query optimizations on highly filtered relational catalogs.
  * **🔗 Repository**: [job-portal](https://github.com/RitikPandey68/job-portal)
</details>

<!-- Project 9 -->
<details>
  <summary><b>🏥 Project 9: Hospital Management System — Medical Records Ledger</b></summary>
  <br>
  
  ### 📌 Overview
  * **Problem Statement**: Healthcare operators must coordinate appointment slots, look up large patient EHR files quickly, and manage ward availability logs securely.
  * **What it does**: An integrated hospital dashboard facilitating medical scheduling, Electronic Health Record search, bed allocations, and billing.
  * **Tech Stack**: `Java`, `Spring Boot`, `JPA/Hibernate`, `PostgreSQL`, `Vanilla HTML5/CSS3/JavaScript`.
  * **Key Features**:
    * **Doctor Roster Scheduler**: Real-time checking of doctor availabilities to book consultation slots.
    * **EHR Search Console**: Complete medical records lookup by patient ID tags.
    * **Bed Occupancy Radar**: Live tracker showing availability levels in ICU, General, and Pediatric wards.
    * **Dynamic Invoice Compiler**: Automatically calculates consultation fees, medicine costs, and ward stay rates.
    * **Doctors Rating Directory**: Review and rank specialists.
  * **Architecture Highlights**:
    * Range-partitions EHR databases by year and month to secure old entries.
    * Composite clustered index keys optimized for patient identification (`patient_id` and `record_date`).
  * **Why this project is valuable**: Demonstrates handling of highly structured, sensitive transactional health records.
  * **🔗 Repository**: [hospital-management](https://github.com/RitikPandey68/hospital-management)
</details>

<!-- Project 10 -->
<details>
  <summary><b>🎬 Project 10: Netflix Clone — High-Performance Streaming UI</b></summary>
  <br>
  
  ### 📌 Overview
  * **Problem Statement**: Media portals need to serve massive binary video payloads and catalog search queries concurrently without putting high loads on standard transactional databases.
  * **What it does**: A premium movie discovery application utilizing object storage, categorized rows, watchlists, and movie details drawer.
  * **Tech Stack**: `Java`, `Spring Boot`, `PostgreSQL` (User lists), `MongoDB` (Reviews), `MinIO` (Object Storage), `Vanilla HTML/CSS/JS`.
  * **Key Features**:
    * **Cinematic Billboard**: Animated landing spotlight banner displaying featured releases.
    * **Horizontal Row Scroll**: Scrolling list rows (Trending, Sci-Fi, Horror) with overlay detail buttons.
    * **HUD Video Player**: Simulated 4K video player HUD with custom timeline indicators.
    * **Custom reviews**: Live movie comment feed.
    * **Watchlist Manager**: Persists user selection items.
  * **Architecture Highlights**:
    * Decouples media storage by offloading binary video payloads directly into MinIO buckets.
    * Uses a hybrid storage model: SQL (PostgreSQL) for user details and NoSQL (MongoDB) for movie review logs.
  * **Why this project is valuable**: Implements best practices for high-volume content delivery networks (CDNs) and database offloading.
  * **🔗 Repository**: [netflix-clone](https://github.com/RitikPandey68/netflix-clone)
</details>

<!-- Project 11 -->
<details>
  <summary><b>📄 Project 11: AI Document Summarizer — RAG-based Document Intelligence</b></summary>
  <br>
  
  ### 📌 Overview
  * **Problem Statement**: Enterprise personnel struggle to parse, digest, and query long-form PDF reports, financial contracts, or legal papers efficiently.
  * **What it does**: A secure document summarization portal where users drag-and-drop documents, select summary formats, and query content via RAG chatbot.
  * **Tech Stack**: `Python`, `FastAPI`, `SQLAlchemy`, `PostgreSQL` (pgvector), `MinIO` (Document Storage), `Vanilla HTML5/CSS3/JavaScript`.
  * **Key Features**:
    * **Upload Zone**: Drag-and-drop file inputs with progress bars and format checking.
    * **Dual-Pane Console**: Side-by-side comparison pane showing original documents next to AI briefs.
    * **Summary Length Toggles**: Auto-summarize content into bullet points, medium briefs, or long reports.
    * **RAG Q&A Console**: Chatbot interface answering context-aware questions about the document.
    * **PDF Report Builder**: Export generated summaries into clean PDF formats.
  * **Architecture Highlights**:
    * Uses pgvector semantic indexing on text chunks.
    * Stores raw source files in partitioned MinIO object buckets.
  * **Why this project is valuable**: Shows end-to-end data pipeline capabilities for structured document analysis.
  * **🔗 Repository**: [ai-document-summarizer](https://github.com/RitikPandey68/ai-document-summarizer)
</details>

<!-- Project 12 -->
<details>
  <summary><b>📊 Project 12: Sales & Revenue Analytics Dashboard — Predictive Data Pipeline</b></summary>
  <br>
  
  ### 📌 Overview
  * **Problem Statement**: Revenue operations departments face challenges with manual data cleaning and lack predictive analytics models for future revenue forecasts.
  * **What it does**: An end-to-end data analytics and predictive ML pipeline displaying sales trends, metrics, and forecasts.
  * **Tech Stack**: `Python`, `Pandas`, `NumPy`, `Scikit-Learn`, `Streamlit`, `MySQL`, `Docker`, `GitHub Actions`.
  * **Key Features**:
    * **Data Ingestion Pipeline**: Auto-ingests, cleans, and standardizes raw sales logs.
    * **ML Forecast Engine**: Trains regression models to predict revenue trends for upcoming quarters.
    * **Streamlit Visual Hub**: Renders interactive graphs, revenue KPIs, and product sales statistics.
    * **CI Pipeline Testing**: Auto-linting and pytests triggered via GitHub actions.
  * **Architecture Highlights**:
    * Containerized workflow isolating database services and streamlit web services using Docker Compose.
    * Modular codebase structure separation: data loaders, model files, and UI scripts.
  * **Why this project is valuable**: Demonstrates core Python data engineering, automated ML validation, and predictive visualization.
  * **🔗 Repository**: [sales-revenue-analytics-dashboard](https://github.com/RitikPandey68/sales-revenue-analytics-dashboard)
</details>

<!-- Project 13 -->
<details>
  <summary><b>⚙️ Project 13: Workflow Automation Backend — Distributed Task Orchestrator</b></summary>
  <br>
  
  ### 📌 Overview
  * **Problem Statement**: Enterprise backends need a resilient orchestration engine to schedule complex background processes, run task chains, and check states.
  * **What it does**: A distributed background workflow controller designed to build automation flows, schedule triggers, and monitor tasks.
  * **Tech Stack**: `Java`, `Spring Boot`, `JPA/Hibernate`, `MySQL`.
  * **Key Features**:
    * **Cron Task Scheduler**: Manages complex cron and delay timers for asynchronous tasks.
    * **Workflow Builder API**: Interfaces to define chains of dependent task blocks.
    * **Job Monitor Dashboard**: Real-time status logs tracking failed, active, and completed jobs.
    * **API Driven Architecture**: Full REST interface for remote system commands.
  * **Architecture Highlights**:
    * Quartz Scheduler implementation managing concurrency limits.
    * Persistent task queues preventing job failures during server restarts.
  * **Why this project is valuable**: Exhibits deep understanding of concurrency, asynchronous tasks, and job status management.
  * **🔗 Repository**: [workflow-automation-backend](https://github.com/RitikPandey68/workflow-automation-backend)
</details>

<!-- Project 14 -->
<details>
  <summary><b>⚡ Project 14: Smart Energy Grid — Usage Optimization Dashboard</b></summary>
  <br>
  
  ### 📌 Overview
  * **Problem Statement**: Utility operations struggle with grid instability, peak-hour overload costs, and lack of real-time usage monitoring widgets.
  * **What it does**: A grid optimization dashboard simulating grid load trends, usage forecasts, and peak pricing strategies.
  * **Tech Stack**: `Vanilla CSS/JS`, `HTML5`, `Java` (Backend REST), `Python` (Analytical scripts).
  * **Key Features**:
    * **Grid Load Visualizer**: Dynamic line charts plotting load distributions.
    * **Peak-Alert Indicators**: Warning prompts triggered when simulated grid loads approach max capacities.
    * **Device Health Tracker**: Status monitor indicating transformer and storage node operations.
    * **Pricing Optimization**: Dynamic cost logic adjusting tariff calculations during high-load periods.
  * **Architecture Highlights**:
    * Glassmorphic responsive panel design displaying data metrics efficiently.
    * Decoupled backend services routing simulated utility grid metrics via REST.
  * **Why this project is valuable**: Applies software engineering paradigms to solve load balancing and resource optimization.
  * **🔗 Repository**: [project-on-the-energy](https://github.com/RitikPandey68/project-on-the-energy)
</details>

---

# 💡 Engineering Principles & Architecture

I build applications with long-term maintainability, security, and scalability in mind:

- **Clean Architecture & SOLID Principles**: Decoupling business logic from infrastructure (controllers, services, repositories) to write modular and testable code.
- **RESTful API Design**: Creating semantic endpoints with structured JSON schemas, proper HTTP codes, authentication filters, and standardized error messaging.
- **Security First**: Securing APIs using JWT, implementing password hashing, and enforcing role-based access control (RBAC).
- **Database Optimization**: Designing query structures with composite indexing, time-series partitioning, and database cache architectures.
- **Microservices & Messaging**: Designing decoupled application services using WebSockets, REST communications, and message queue patterns.

---

# 📊 GitHub Analytics

<div align="center">
  <table border="0">
    <tr>
      <td>
        <img height="180em" src="https://github-readme-stats.vercel.app/api?username=RitikPandey68&show_icons=true&theme=tokyonight&count_private=true" alt="GitHub Stats"/>
      </td>
      <td>
        <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=RitikPandey68&layout=compact&theme=tokyonight" alt="Top Languages"/>
      </td>
    </tr>
  </table>
  
  <br>
  
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=RitikPandey68&theme=tokyonight" alt="GitHub Streak"/>
  
  <br>
  
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=RitikPandey68&theme=tokyo-night" width="100%" alt="Activity Graph"/>
</div>

---

# 🏅 GitHub Achievements & Trophies

<div align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=RitikPandey68&theme=algolia&margin-w=10&row=2&column=4" alt="GitHub Trophies"/>
</div>

---

# 🏆 Certifications & Badges

- **AWS Certified Cloud Practitioner** — Amazon Web Services (AWS)
- **AWS Academy Cloud Foundations** — AWS Academy
- **Google Cloud Innovators Program Member** — Google Cloud
- **Java Programming Certification** — Oracle / Authorized Partner
- **Python Programming Certification** — Python Institute / Authorized Partner

---

# 🌱 Currently Learning & Exploring

- **Container Orchestration**: Kubernetes (K8s) deployments, service meshes, and Helm charts.
- **Distributed Pipelines**: Event-driven architectures using Apache Kafka and RabbitMQ.
- **System Design**: Advanced system design patterns (caching strategies, rate limiters, database replication, and consensus protocols).
- **CI/CD Automation**: Writing robust automation workflows using GitHub Actions and Jenkins.

---

# 📫 Connect With Me

* 💼 **LinkedIn**: [linkedin.com/in/ritik-pandey-sde/](https://linkedin.com/in/ritik-pandey-sde/)
* 📧 **Email**: [ritikpandeycode@gmail.com](mailto:ritikpandeycode@gmail.com)
* 📞 **Phone**: [+91 7562806146](tel:+917562806146)
* 📍 **Location**: Bengaluru, India
* 🐙 **GitHub**: [github.com/RitikPandey68](https://github.com/RitikPandey68)
* 💻 **LeetCode**: [leetcode.com/u/ritikpandeycode/](https://leetcode.com/u/ritikpandeycode/)
* 🏅 **HackerRank**: [hackerrank.com/profile/pandeyritikkuma1](https://www.hackerrank.com/profile/pandeyritikkuma1)

---

<div align="center">
  <h3>⭐ Thanks for visiting my profile ⭐</h3>
  <p>Let's build something amazing together 🚀</p>
  <img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/aqua.png" width="100%">
</div>
